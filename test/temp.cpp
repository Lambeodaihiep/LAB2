...
float error, P, I, D;
float kp1 = 110.0, ki1 = 0.002, kd1 = 10;
float sum_error = 0, prev_error = 0, dt = 0.002;

int PWM_value, dir;

void kalman_1d(float KalmanState, float KalmanUncertainty, float KalmanInput, float KalmanMeasurement) {
  KalmanState=KalmanState+0.004*KalmanInput;
  KalmanUncertainty=KalmanUncertainty + 0.004 * 0.004 * 10 * 10; // 10 độ / giây
  float KalmanGain=KalmanUncertainty * 1/(1.0*KalmanUncertainty + 3 * 3);
...
void _PID(){
  ...
  error = KalmanAngleRoll - First_angle;
  sum_error += error;
...
void controlMotor(){
  ...
  //    vl = 0; vr = 0;
  //   }
  int abs_error = fabs(error);
  if (abs_error < 2.1) // nếu xe về vị trí giữa, sai số quá nhỏ thì vẫ cho xe di chuyển làm con lắc lắc về 2 phía
  {
    vl = 100;
    vr = 80;
  }
  else 
    {vl = PWM_value; vr = PWM_value - 30;}
  setMotor(dir, vl, vr);  
