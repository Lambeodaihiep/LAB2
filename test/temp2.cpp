...
float error, P, I, D;
float kp1 = 80.0, ki1 = 0.002, kd1 = 0.8;
...
void loop() {
  ...
  Serial.print(" PID value: "); Serial.println(P + I + D);
  int PID = int(P + I + D);
  ...
  if(fabs(error) < 1 || fabs(error) > 10) {
    setMotor(0 ,0);
  }
  else if (fabs(error) < 3 && fabs(error) > 1)
  {
    if (error > 0)
    {
      setMotor(-1, 150);
